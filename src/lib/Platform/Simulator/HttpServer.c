...
static bool WriteAll(int socketHandle, const char *data, size_t length) {
    ...
    while (written < length) {
        ssize_t result = send(socketHandle, data + written, length - written, MSG_NOSIGNAL);
...
static bool WriteResponse(int socketHandle, const HttpResponse *response) {
    ...
    int headerLength = snprintf(header, sizeof(header),
                                ...
                                response->ContentType == NULL ? DEFAULT_CONTENT_TYPE : response->ContentType,
                                (unsigned) response->BodyLength);
...
static bool ReadRequest(int socketHandle, size_t *totalRead, size_t *bodyOffset, size_t *bodyLength) {
    ...
    while (received < capacity) {
        ssize_t result = recv(socketHandle, requestBuffer + received, capacity - received, 0);
...
static void ServeConnection(int socketHandle) {
    ...
    char *cursor = requestBuffer;
    char *methodEnd = strchr(cursor, ' ');
    ...
    *pathEnd = '\0';
    ...
    bool methodKnown = ParseMethod(requestBuffer, &method);
