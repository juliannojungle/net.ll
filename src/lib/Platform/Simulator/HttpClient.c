...
/* A library must not identify itself as one particular application. The OSM tile
 * policy asks for an identifiable agent, so a consumer that needs its own string
 * overrides NET_LL_USER_AGENT from its build. */
#ifndef NET_LL_USER_AGENT
#define NET_LL_USER_AGENT "net.ll/0.alpha (Simulator)"
#endif
#define USER_AGENT NET_LL_USER_AGENT
